...
// Model for Customers entity
    models.Customers = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-DZ13-BogdanPetrovic4382/webresources/entity.customers/",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/CS230-DZ13-BogdanPetrovic4382/webresources/entity.customers/';
            }
    ...
    // Collection class for Customers entities
    models.CustomersCollection = Backbone.Collection.extend({
        model: models.Customers,
        url: "http://localhost:8080/CS230-DZ13-BogdanPetrovic4382/webresources/entity.customers/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CustomersCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-customers-list-itemis template identifier for item
                        templateName: '#tpl-customers-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.customers = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-customers-details is template identifier for chosen model element
                    templateName: '#tpl-customers-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Customers();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-customers-details is a template identifier for chosen model element
                templateName: '#tpl-customers-details',
